...
public class Puzzle4
{
	...
	{
		String input = "[1518-04-22 00:56] falls asleep\n" +
	...
	static void sortInputChronologically(List<String> list)
	{
		list.sort(Comparator.naturalOrder());
	}
	
	static Map<Integer, Map<Integer, Integer>> getSleepTimesStruct(List<String> inputData)
	{
		...
		for (String record : inputData)
		{
			...
			if (record.contains("begins"))
				currId = Integer.parseInt(record.substring(record.indexOf('#') + 1,
						record.indexOf('b') - 1));
			else if (record.contains("wakes"))
			{
				currAwakeningMin = currMinIndex - 1;
				currTotalMinsCount = currAwakeningMin - currFallenAsleepMin;
				
				final var minutes = sleepTimes.get(currId) == null
				...
				sleepTimes.put(currId, minutes);
			} else if (record.contains("falls"))
				currFallenAsleepMin = currMinIndex;
		}
		
		return sleepTimes;
	}
	
	static void printFinalStruct(Map<Integer, Map<Integer, Integer>> totalSleepTimes)
	{
	...
	static int getFinalResult(Map<Integer, Map<Integer, Integer>> map)
	{
		final int[] currWinnerId = new int[1];
		final int[] currMaxTime = new int[1];
		final int[] currMinuteWinner = new int[1];
		
		map.forEach((k, v) ->
		{
			final var currTotalTime =
					Arrays.stream((v.values()
							.stream()
							.mapToInt(Integer::intValue)
							.toArray()))
							.sum();
			
			if (currTotalTime > currMaxTime[0])
			{
				currMaxTime[0] = currTotalTime;
				currWinnerId[0] = k;
				
				@SuppressWarnings("OptionalGetWithoutIsPresent")
				var currMaxMinutesSlept =
				...
				{
					if (entry.getValue() == currMaxMinutesSlept)
					{
						currMinuteWinner[0] = entry.getKey();
						break;
					}
				}
			}
		...
		for (var entry : map.entrySet())
		{
			...
			if (innerEntry.getValue() > bestMinCount)
			{
				bestMinCount = innerEntry.getValue();
				bestMin = innerEntry.getKey();
				newBestId = entry.getKey();
			}
		}
		
		return newBestId * bestMin;
	}
	
	public static void main(String[] args)
	{
		var nightShiftsData = new ArrayList<String>();
		
		fakeInput();
		try (var input = new Scanner(System.in))
		{
			while (input.hasNextLine())
				nightShiftsData.add(input.nextLine().trim());
		}
		sortInputChronologically(nightShiftsData);
		
		final var map = getSleepTimesStruct(nightShiftsData);
		
		printFinalStruct(map);
		System.out.println(getFinalResult(map));
	}
}
